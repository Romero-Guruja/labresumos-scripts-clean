<?php
/**
 * Plugin Name: Lab Resumos - AutoLogin
 * Description: Magic link de autologin (login automático + redirect pro checkout) usado
 *              pela recuperação de vendas e pelo acesso de cortesia. Fase F2 do roadmap
 *              (docs/plugins-custom-analise-e-roadmap.md) — portado do snippet WPCode #1241
 *              ("AutoLogin DEBUG VERSION" v7), sem alteração de lógica. Ao logar no
 *              WordPress, o Edwiser Bridge (SSO, não tocado por este plugin) leva o
 *              usuário logado ao Moodle.
 * Version: 1.0.0
 * Author: Lab Resumos
 */
...
// FUNÇÕES AUXILIARES
// ============================================================================
if (!function_exists('lr_generate_autologin_token')) {
...
if (!function_exists('lr_get_all_tokens')) {
...
if (!function_exists('lr_delete_token')) {
    function lr_delete_token($token) {
        return delete_option('lr_autologin_' . $token);
    }
}

if (!function_exists('lr_delete_expired_tokens')) {
...
if (!function_exists('lr_get_pending_orders')) {
...
if (!function_exists('lr_get_whatsapp_message_template')) {
...
if (!function_exists('lr_save_whatsapp_template')) {
...
if (!function_exists('lr_payment_links_admin_page')) {
